...
- SWMM5 Engines tab with 34 engines total:
  - 33 Live engines: EPA SWMM5 (C/Python backend), JavaScript (browser-native), Rust/WASM (140KB browser binary), Julia, CUDA GPU, MATLAB, WAT/WASM, Go API Server, Ruby, Nim, Delphi, R, Zig, Kotlin, Swift, Mojo, Ada, Chapel, C#, Fortran, AutoLISP, Common Lisp, Clojure, Scheme/Racket, Hy, VBA, Lua, Tcl, Haskell, Scala, Dart, Elixir, OCaml
...
- SWMM5 Engines tab uses Express backend (port 3001) proxied through Vite for EPA engine; JS and Rust/WASM engines run client-side in browser
