...
- SWMM5 Engines tab with 37 engines total:
  - 7 Real engines: EPA SWMM5 (C/Python backend), JavaScript (browser-native), Rust/WASM (140KB browser binary), Go (native 8.6MB binary on port 3002), Pure Python (stdlib-only on port 3003), C standalone (30KB binary on port 3004), C++ OOP (75KB binary on port 3005)
...
- **Structure**: 
  ...
  - `src/apps/SwmmEngineRunner.jsx` — SWMM5 Engines tab: 37 engine options (36 live + 1 concept), upload .inp files, run simulations, view/download .rpt reports
...
- **Port**: 5000 (Vite dev server), 3001 (Express backend API), 3002 (Go engine), 3003 (Python engine), 3004 (C engine), 3005 (C++ engine)
- **Deployment**: Vite frontend + Express backend + 4 engine child processes (all started via npm run dev)
...
- SWMM5 Engines tab: 7 real engines (EPA C, JS browser, Rust/WASM, Go native, Pure Python, C standalone, C++ OOP), 29 proxy engines (route to EPA C with rebranding), 1 concept (PySWMM)
